...
void ExecuteTI() {
    ...
    strcat(exePath, "00 bot_escreverchamado.exe");

    if (GetFileAttributes(exePath) != INVALID_FILE_ATTRIBUTES) {
        ShellExecute(NULL, "open", exePath, NULL, NULL, SW_SHOWNORMAL);
        exit(0);
    } else {
        MessageBox(NULL, "O arquivo 00 bot_escreverchamado.exe não foi encontrado na mesma pasta.", "Erro", MB_OK | MB_ICONERROR);
    }
}
